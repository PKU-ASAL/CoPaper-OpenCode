...
2. **Agent Skills (`.agents/skills/`)**：22 个 OpenCode/Sisyphus 技能，通过自然语言触发，覆盖从研究主线构建到 LaTeX 转换的完整写作流程。
3. **CLI (`vibe` 命令)**：基于 Click 的命令行工具，用于项目初始化、状态查询、显式阶段状态更新、阶段跳过、Git 提交/回滚、周报和差异对比。

核心理念：**结构与模板优先，AI 辅助优化**。AI 基于你提供的 Insight 和实验数据组织语言、优化文字，而非自由发挥。
