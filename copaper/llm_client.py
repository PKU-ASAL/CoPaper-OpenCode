...
``COPAPER_MODEL`` from the environment. The ``TokenBucket`` rate limiter
...
def resolve_model(override: str | None = None) -> str:
    """Pick the model name: explicit override > ``COPAPER_MODEL`` env."""
    if override:
        candidate = override.strip()
        if candidate:
            return candidate

    env_value = (os.environ.get("COPAPER_MODEL") or "").strip()
    if not env_value:
        raise LLMConfigError(
            "COPAPER_MODEL is not set. Configure it in .env, e.g. "
            "COPAPER_MODEL=gpt-4o-mini."
        )
    return env_value
