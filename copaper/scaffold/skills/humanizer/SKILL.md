...
| Input | Required | When to Read | Purpose |
|-------|----------|--------------|---------|
| Text to humanize | **Required** | Start | Source prose to edit for natural, human-written voice |
| Writing sample/reference file | Optional | Before rewriting, if provided | Calibrate sentence rhythm, word choice, and tone |
...
If the user explicitly asks to record artifact readiness after humanization, restate the artifact, status, confidence, evidence, and reason, wait for confirmation, then route the record action to `@copaper-recorder` so it can call `copaper_artifact_record`.
