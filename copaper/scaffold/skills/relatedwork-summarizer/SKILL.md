...
- Call `copaper_relatedwork_status` first. This tool is read-only and reports which papers are downloaded and which already have summaries; do not replace it with prompt-only status text or manual state inspection.
...
- Run `copaper --root . relatedwork build-index`. The CLI scans `relatedwork/papers/*.md`, updates `.agents/cross_index.json`, and emits a coverage report (covered points, gaps, ratio) against `storyline.md`.
...
- Call `copaper_relatedwork_status` before the final response if you need to report final literature, summary, BibTeX, or cross-index counts.
