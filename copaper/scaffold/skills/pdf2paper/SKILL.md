...
- Text-based `pdf`: supported through `copaper_pdf_extract`
- Scanned/image-only `pdf`: not OCRed by this skill; if `copaper_pdf_extract` reports low confidence or little/no text, stop and ask the user for a text-based PDF or extracted text
...
5. Do not run a separate shell/file existence check for the `.pdf`; `copaper_pdf_extract` performs this validation in Step 2.
6. Check whether `paper.md` exists through `copaper_paper_structure_status` in Step 3:
...
1. Call `copaper_paper_structure_status` to inspect `paper.md` structure and Level 5 writing targets.
...
- **NEVER** pretend OCR was performed by this skill; disclose low-confidence or missing text from `copaper_pdf_extract`
