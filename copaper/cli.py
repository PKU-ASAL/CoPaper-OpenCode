"""CLI entry point for the `copaper` command."""
...
import copaper
from copaper.constants import PHASE_ORDER, PhaseStatus
from copaper.eventlog import EventLogger
from copaper.literature import LiteratureCatalog, LiteratureCatalogError
from copaper.scaffold import scaffold_project
...
def _load_state_manager_or_exit(root: str) -> StateManager:
    ...
    try:
        ...
        click.echo("Error: No project found. Run 'copaper init' first.", err=True)
        sys.exit(1)
    return sm
...
@click.version_option(version=copaper.__version__)
@click.pass_context
def main(ctx: click.Context, root: str) -> None:
    """CoPaper - AI-assisted academic writing framework."""
...
def init(ctx: click.Context, name: str, domain: str) -> None:
    """Initialise a new CoPaper project."""
    root = ctx.obj["root"]
    sm = StateManager(root)
...
def status(ctx: click.Context, as_json: bool) -> None:
    ...
    try:
        ...
        click.echo("No project found. Run 'copaper init' first.", err=True)
        sys.exit(1)
...
def checkers_collect(
    ...
    from copaper.checker_integration import CheckerHarness
...
def checkers_status(ctx: click.Context, as_json: bool) -> None:
    ...
    from copaper.checker_integration import CheckerTracker, format_checker_results
...
def relatedwork_search_cmd(
    ...
    click.echo(
        f"Next: copaper --root {root} relatedwork import --input {outcome.cache_path}"
    )
...
def relatedwork_download_cmd(
    ...
    from copaper.relatedwork_download import download_papers
...
def relatedwork_keywords_cmd(
    ...
    from copaper.llm_client import LLMConfigError
    from copaper.relatedwork_keywords import extract_keywords
    ...
    for query in outcome.queries:
        click.echo(f"  - {query}")
    click.echo(
        f"Next: copaper --root {root} relatedwork search "
...
@click.option("--model", default=None, help="Override COPAPER_MODEL.")
@click.option(
    "--qps",
    type=float,
    default=16.0,
...
def relatedwork_summarize_cmd(
    ...
    from copaper.llm_client import LLMConfigError
    from copaper.relatedwork_summarize import summarize_papers
...
def relatedwork_clean_cmd(
    ...
    from copaper.relatedwork_clean import clean_relatedwork, plan_targets
...
def report(ctx: click.Context, since: str | None, output: str | None) -> None:
    ...
    from copaper.report import generate_weekly_report
...
def diff_cmd(ctx: click.Context, phase_a: str, phase_b: str) -> None:
    ...
    from copaper.report import generate_diff_report
