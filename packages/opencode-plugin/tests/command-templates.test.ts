...
describe("command templates", () => {
  ...
  test("renders /vibe-relatedwork in Chinese by default", () => {
    ...
    expect(output).toContain("只读工具（无需确认）：`vibepaper_relatedwork_status`")
    expect(output).toContain("写盘工具（必须复述完整参数，等待用户明确确认后才可调用）：`vibepaper_relatedwork_keywords`")
  ...
  test("renders /vibe-relatedwork in English when requested", () => {
    ...
    expect(output).toContain("vibepaper_relatedwork_status")
    expect(output).toContain("vibepaper_relatedwork_keywords")
    expect(output).toContain("Read-only tools (no confirmation required): `vibepaper_relatedwork_status`")
    expect(output).toContain("Write tools (must restate every argument and wait for explicit user confirmation before calling): `vibepaper_relatedwork_keywords`")
