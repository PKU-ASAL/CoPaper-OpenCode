...
import { isVibePaperAgentName, type VibePaperAgentName } from "./agent-profiles"
...
export interface VibePaperConfigDefaults {
  model?: string
  temperature?: number
}

export interface VibePaperAgentOverride {
  enabled?: boolean
  model?: string
  temperature?: number
  promptAppend?: string
...
export interface VibePaperProjectConfig {
  schemaVersion: typeof SCHEMA_VERSION
  locale: Locale
  defaults: VibePaperConfigDefaults
  agents: Partial<Record<VibePaperAgentName, VibePaperAgentOverride>>
}

export interface VibePaperConfigLoadResult {
  path: string
  config: VibePaperProjectConfig
  diagnostics: AgentDiagnostic[]
}

export function loadVibePaperConfig(root: string): VibePaperConfigLoadResult {
...
function withSafeConfig(path: string, diagnostics: AgentDiagnostic[]): VibePaperConfigLoadResult {
  return {
    path,
    config: safeConfig(),
    diagnostics,
...
function parseDefaults(value: unknown, diagnostics: AgentDiagnostic[]): VibePaperConfigDefaults {
  ...
  const defaults: VibePaperConfigDefaults = {}
...
function parseAgents(
  value: unknown,
  diagnostics: AgentDiagnostic[],
): Partial<Record<VibePaperAgentName, VibePaperAgentOverride>> {
  if (value === undefined) return {}
  if (!isPlainObject(value)) {
  ...
  const agents: Partial<Record<VibePaperAgentName, VibePaperAgentOverride>> = {}
  for (const [agentName, rawOverride] of Object.entries(value)) {
    if (!isVibePaperAgentName(agentName)) {
